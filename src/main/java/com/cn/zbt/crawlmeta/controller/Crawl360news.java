...
public class Crawl360news {
	private static final Logger logger = Logger.getLogger(Crawl360news.class);
	...
	public Document fetch(String url, String q, String p) {
		...
		try {
			...
			conn.data("q", q).data("tn", "news").data("rank", "pdate")
	...
	public Document fetch_old(String url, String q, String p) {
		...
		try {
			...
			logger.error("jsoup访问360异常==========>", e);
			logger.error("360异常URL为：" + url);
		} finally {
			return doc;
		}
	}
	...
	public void getData(Document doc, String keyword) {
		...
		Elements elements = doc.select("#news>li>h3");
		for (int p = 0; p < elements.size(); p++) {
			...
			for (Element elementsurl : elementsurls) {
				...
				try {
					Document doc1 = new Crawl360news().fetch(url);
	...
	public static void main(String[] args) {
		...
		Crawl360news sw = new Crawl360news();
		sw.runInter();
		// sw.getDoc("步长制药");
