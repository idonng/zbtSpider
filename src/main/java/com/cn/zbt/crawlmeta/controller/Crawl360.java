	...
	public Document fetch(String url, String q, String p) {
		...
		try {
			...
			conn.data("q", q).data("fr", "tab_www").data("src", "srp_paging")
					.data("adv_t", "w").data("pn", p).timeout(5000);
			doc = conn.get();
		} catch (IOException e) {
		...
		try {
			conn = Jsoup
					...
					.header("User-Agent",
							"Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/54.0.2840.99 Safari/537.36");
			conn.data("q", q).data("fr", "tab_www").data("src", "srp_paging")
					.data("adv_t", "d").data("pn", p).timeout(5000);
			doc = conn.get();
		} catch (IOException e) {
			logger.error("jsoup访问人民网异常==========>", e);
			logger.error("360网异常URL为：" + url + " 关键词:" + q + " 页码：" + p);
		} finally {
			return doc;
		}
	}
	...
	public void getDoc(String keyword) {
		...
		do {
			...
			try {
				...
				if (doc == null || doc.toString().contains("360搜索，SO靠谱")
						|| doc.toString().contains("抱歉，未找到")) {
					continue;
				}
				if (doc.toString().contains("以下是该时间范围外的结果")) {
					reg = "";
				} else if (doc.toString().contains("page")) {
				...
				e.printStackTrace();
				logger.error("解析错误URL:" + url + "。关键字为：" + keyword + "。异常详情："
						+ e);
			}
	...
	public void getData(Document doc, String keyword) {
		...
		for (int p = 0; p < elements.size(); p++) {
			...
			for (Element elementsurl : elementsurls) {
				...
				if (url.contains("360webcache") || url.contains("360haoyao")
						|| url.contains("mingpian")) {
					// logger.info("包含360webcache，跳过URL" );
					continue;
				}
				...
				try {
					...
					resultTabService.insertRes(CommonUtils.setMD5(url), title,
							url, content, resultSource, type, keyword,
							Integer.valueOf(pls), Integer.valueOf(zfs),
							pubdate, sinatime_now, author, sinatime_now,webConfKey);
